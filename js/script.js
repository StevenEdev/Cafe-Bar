...
function eventListeners(){
  ...
  document.addEventListener('change', (e) => { obtenerEvento(e) });
  document.addEventListener('keyup', (e) => { obtenerEvento(e) });
...
function leerLocalStorageCompra(){
  ...
  productosLS.forEach(function (item){
      ...
      row.innerHTML = `
          ...
          <td>${item.precio}</td>
          <td> 
              <input type="number" id="tex" class="form-control cantidad" min="1" value=${item.cantidad}> 
              
          </td>
          <td id='subtotales'>${item.precio * item.cantidad}</td>
          <td>
              <a href="#" id="basura" class="borrar-producto" style="font-size:30px" data-id="${item.id}"><i class="bi bi-trash"></i></a>
          </td>
      `;
 ...
 function calcularTotal(){
  ...
  let valorfinal = localStorage.setItem("ValorF",document.getElementById('total').value);
}
